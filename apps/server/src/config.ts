...
export async function writeCodexConfig(config: AppConfig): Promise<void> {
  ...
  const providerBaseUrl = config.llmProxyEnabled
    ? config.gatewayUrl + "/llm"
    : config.arkBaseUrl;
  const providerEnvKey = config.llmProxyEnabled ? "AGENT_TOKEN" : "ARK_API_KEY";
  const toml = [
    ...
    "env_key = " + JSON.stringify(providerEnvKey),
    'wire_api = "responses"',
