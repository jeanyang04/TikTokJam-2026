...
## 0. Decisions log (grilled 2026-08-26 — these override anything below that disagrees)

| # | Decision |
|---|---|
| D1 | **IFC (Scene 2) is committed**, not stretch. Taints start Day 2; egress block Day 3 AM. NL grant is the only stretch item. |
| D2 | Gateway transport = **MCP streamable HTTP** (`@modelcontextprotocol/sdk`). REST `POST /gw/:tool` is a documented fallback only. **Spike verified:** codex 0.144.6 `exec` sends `Authorization` from `-c mcp_servers.x.http_headers` on `initialize`. No `env_http_headers` fallback needed. |
| D3 | Approval flow = immediate `DENIED … pending approval` result; operator decides; **user sends a follow-up message**; next turn succeeds. No long-poll. |
| D4 | **RLS is owner-only.** No `policy_grants_mirror`, no dual write. Grants are enforced in the gateway only. `withOwner()` binds `app.owner_id` from the verified token's `own` (or the checked grant's `fromOwner`), never from a tool argument — one audited function, tested. |
| D5 | Two protected resources: **workspaces** (files, LOCK 1) and **crm_records** (Postgres, LOCK 1 + LOCK 2). |
| D6 | **No hosted instance.** Judges reproduce locally with their own Ark key. LLM proxy is optional (after Sync 2, cut second). |
| D7 | Grants are **intra-tenant only**; `createGrant` → 400 on owner mismatch. Cross-tenant sharing = "next steps". |
| D8 | Card buttons: **Allow for this run / Always allow / Deny.** "For this run" widens `RunToken.scp` and (for grant cards) writes a grant with `expiresAt` = run token expiry. |
| D9 | **Taints persist after grant revoke**; a revoked grant's taint becomes `egress: []` — revocation narrows, never widens. |
| D10 | Egress classes: **internal / agent / external**. Tools: `workspace_read`, `workspace_write`, `crm_read`, `crm_write`, `webhook_send`. `share_to_agent` dropped — `workspace_write` to another agent's workspace *is* the `agent` class. |
| D11 | **Pattern card cut.** No `pattern.ts`, no threshold. Card `source ∈ {live_deny, nl_intent}`; card `kind ∈ {scope, grant, declassify}`. |
| D12 | **NL grant kept as stretch** (B1, Day 3, after Sync 2), regex fallback for the demo phrase mandatory. |
| D13 | Seeded no-password login is fine; README sentence: identity mocked as the brief permits, every authz decision server-side and tested, real IdP swaps in at `auth.ts`. |
| D14 | Scene 2 = **try live once, `/demo/replay` as fallback, say which happened.** Replay fires the real gateway calls with the real token. |
| D15 | Human API: **404 unknown ID / 403 not-yours + RunEvent.** 404s are not logged. |
| D16 | Buttons: **Revoke** per grant (Scene 5), **Kill switch** per agent (all tokens revoked, tools cleared), kit's **Stop** unchanged (process). |
| D17 | Timeline captures everything (gateway, card, grant, Codex command/file/mcp, llm); UI filter defaults to "policy only". |
| D18 | Day-2 reshuffle: Zeon +taints; B3 +fingerprint index; B2 takes `/demo/replay` + webhook sink; B1 loses pattern.ts; F moves Scene 2 rendering up. Sync 2 gate = Scenes 1/4/5/6 live **and** a grant-gated read writes a taint. |
...
interface ApprovalRequest { id; source:"live_deny"|"nl_intent"; kind:"scope"|"grant"|"declassify"; agentId; ownerId; runId?; resource; action; status:"pending"|"allow_run"|"allow_always"|"denied"; createdAt; decidedAt?; decidedBy? }
...
Postgres `migrations/001_init.sql`: roles `app_admin`, `app_agent NOBYPASSRLS`; `crm_records(id, owner_id, customer, note, updated_at)`; `ENABLE`+`FORCE ROW LEVEL SECURITY`; policy `USING/WITH CHECK (owner_id = current_setting('app.owner_id', true))` — **owner-only (D4)**; grants are gateway-enforced. Seed: Jean 2 rows, Alex 1 row.
...
Tools: `workspace_read{agent,path}`, `workspace_write{agent,path,body}`, `crm_read{customer?}`, `crm_write{customer,note}`, `webhook_send{url,body}` (external).
...
| 7 | Grants (create, revoke, live-check per call, intra-tenant only) | Zeon | `grants.ts` | allow → revoke → same token 403; other tools 200 |
...
| 12 | LLM proxy (optional, after Sync 2, cut second) | B2 | `llm-proxy.ts`, `config.ts` | human JWT→401; revoked→401; upstream gets real key |
| 13 | IFC (**committed**): taints Day 2, egress check + declassify card Day 3 AM, fingerprint | Zeon + B3 | `ifc.ts` | read under grant → webhook_send 403 ifc w/ origin; own-workspace write 200 |
| 14 | `/demo/replay` + webhook sink + seed | B2 + B1 | `demo.ts`, `scripts/seed.ts` | Scene 2 reproducible |
| 15 | NL grant parse (only stretch item; regex fallback mandatory) | B1 | `app.ts` route | parse → pending card; ownership re-check rejects foreign refs |
...
| Non-owner touches an existing agent/resource | **403 + RunEvent**; unknown ID → 404 (not logged); listings filtered |
...
1 deny → card → Always allow → follow-up message (0:30) · 2 injection → provenance block, live once then replay (0:50) · 3 NL grant card (0:20) · 4 Alex: absent list, 403 logged (0:30) · 5 revoke grant mid-task (0:20) · 6 timeline + still usable (0:20).

## 9. Risks

`http_headers` in `codex exec` **verified 2026-08-26** (codex 0.144.6) · `host.docker.internal` on Linux/Podman → `--add-host`/`host.containers.internal` · MCP SDK vs Fastify → REST fallback `POST /gw/:tool` · JSON store write amplification → batch events, cap 500/run · six scenes with two live runs in 3:00 → pre-seed, `/demo/replay`, fallback video · approval with non-interactive Codex → follow-up message path, documented.
