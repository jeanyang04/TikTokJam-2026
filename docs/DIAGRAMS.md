...
```mermaid
stateDiagram-v2
    [*] --> queued: sendMessage() — estimate task scopes, mint RunToken<br/>(scp = standing ∩ estimate ∪ live tempScopes; taints carried same-thread;<br/>a missing scope raises a run:&lt;scope&gt; card BEFORE the run starts)
    queued --> running: container spawned (menu = scp ∪ withheld)
    running --> running: tool allowed — RunEvent allow (tainting reads add a taint)
    running --> running: tool denied — RunEvent deny, card(live_deny) created
    running --> running: card decided — Allow for this run widens scp/tempScopes or egressAllow /<br/>Always allow writes PolicyGrant or permissions.tools
    running --> running: grant revoked — next grant-gated call denied, its taint → egress [], other tools continue
    running --> completed: agent_message — output screened, threadId backfilled, token expires naturally
    running --> failed: kill switch — every token revoked, tools + tempScopes cleared,<br/>pending cards denied, every call 403/401
    ...
    note right of running
        ...
        Taints outlive the run: the next mint on the same
        Codex thread carries them forward. egressAllow does not —
        a human approved one destination for one run.
    end note
```
...
flowchart LR
    subgraph HOST["Fastify server (trusted)"]
        ...
        FP["fingerprint index<br/>in-memory hot cache, persisted to the store<br/>as hashes + labels — never content<br/>(rehydrated at startup: loadFingerprints)"]
    end
    subgraph BR["Browser"]
    ...
    subgraph CT["Runtime container (untrusted)"]
        ...
        DATA["grant-scoped data<br/>(model can read it — tracked by taint + trust)"]
        NO1["✘ no ARK_API_KEY"]
        NO2["✘ no human JWT"]
        NO3["✘ no JWT_SECRET"]
    end
    subgraph LOG["RunEvents / logs"]
        RD["redact(): Bearer…, eyJ…, ep-…, ARK_* → [redacted]<br/>patterns run BEFORE truncation<br/>no payload bodies, only hashes + origin labels"]
    end
    ...
    DATA -- "can only leave via gateway tools or chat output<br/>→ taint / integrity / output screen" --> HOST
...
Known gap, on purpose rather than by accident: laundering through storage. A run may copy a borrowed file into its *own* workspace (`internal`, permitted), and own-workspace reads are never tainted — so a fresh conversation can read the copy clean. Closing it needs the label **on the file** (`workspace_write` recording taints against the path, `workspace_read` re-applying them), which is not built.
...
|---|---|---|
| 1 Deny-by-default → live card → Allow always | 3, 5 | gateway grant check → card → `PolicyGrant` |
| 2 Injection → provenance block | 4, 8, 8a | run-level taint + trust + fingerprint on outbound tools; output screen on chat |
| 3 NL grant card (stretch) | 5 | same card, same confirm |
| 4 Cross-tenant 403 + RLS | 6 | ownership preHandler (403 + audit), gateway owner check, RLS |
| 5 Grant revoke mid-task | 7, 9 | `PolicyGrant.revokedAt`, checked per call |
| 6 Audit timeline | 8 (ST), 7 (last calls) | every branch writes `human → agent → action → resource → outcome` |

**Scene 2 rehearsal note:** the task narrowing (diagram 2) strips `webhook:send` from the token unless the user's prompt names the send — word the Scene 2 prompt "summarise Writer's notes **and post the summary to our webhook**", or the injected instruction is never attempted at all (silence, not a deny). Alternatively run with `PERMISSION_ESTIMATOR_ENABLED=false`, or script the scene around the output screen (8a), which fires regardless.
